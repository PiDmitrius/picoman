...
func TestLivePlatformAPI2TLS(t *testing.T) {
	if os.Getenv("PICOMAN_LIVE_MAX_TLS") != "1" {
		t.Skip("set PICOMAN_LIVE_MAX_TLS=1 to probe the live MAX endpoint")
	}
	_, err := New("invalid-token-for-tls-probe").GetMe(context.Background())
	if err == nil {
